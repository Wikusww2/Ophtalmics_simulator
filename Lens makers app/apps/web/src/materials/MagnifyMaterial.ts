...
export function makeMagnifyMaterial() {
  const uniforms: MagnifyUniforms = {
    ...
    opacity: { value: 1.0 },
    edgeFeather: { value: 0.02 }, // radial edge feather (0..0.1)
    minusEdgeStrength: { value: 0.0 }, // disabled by default; CPU enables for minus designs only
    minusEdgeMax: { value: 0.0 },      // disabled by default; CPU enables for minus designs only
    blurMaxPx: { value: 2.0 },        // cap on anisotropic blur radius in pixels
  }
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        vec2 dirPower = vec2(dirAxis.y, -dirAxis.x); // perpendicular to axis (flipped)
        float projP = dot(q, dirPower);
        // Refraction offset in pixel units: inward for plus, outward for minus
        // Negative sign ensures clinical convention: +D magnifies (inward), −D minifies (outward)
        ...
        // Darken only when spherical equivalent is minus and displacement is strictly outward
        float outward = (dot(offsetPx, q) > 1e-6) ? 1.0 : 0.0; // exclude zero to avoid plus-lens darkening
